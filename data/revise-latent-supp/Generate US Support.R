...
### Major Powers
### McManus and Nieman 2019: Edited by Joshua Alley
...
log_us_arms+log_us_troops+us_vis+us_ex+log_us_words+us_nukes
...
factors=1, data=d,
lambda.constraints = list(log_us_troops=list(2,"+")),
...
phi.us
colnames(phi.us) <- c("low95","low90","median","high90","high95","mean","sd")
rownames(phi.us) <- sub("\\phi.", "", rownames(phi.us))


# clean
phi.us <- as.data.frame(phi.us)
phi.us$year <- as.numeric(gsub(".*?([0-9]+).*", "\\1", rownames(phi.us)))
phi.us$country <- gsub('[[:digit:]]+', '', rownames(phi.us))
phi.us$country <- gsub(".", "", phi.us$country, fixed = TRUE)
phi.us$ccode1 <- 2

# Save lambda/psi as one .csv file and save country ideal points as another .csv file
#write.csv(lambda,file="data/revise-latent-supp/lambda_us.csv")
# write.csv(phi.us, file="data/revise-latent-supp/country_irt_us.csv")
