...
### Major Powers
### McManus and Nieman: Revised by Joshua Alley
...
log_fra_arms+log_fra_troops+fra_vis+fra_ex
...
factors=1, data=d,
lambda.constraints = list(log_fra_troops=list(2,"+")),
...
phi.fr
colnames(phi.fr) <- c("low95","low90","median","high90","high95","mean","sd")
rownames(phi.fr) <- sub("\\phi.", "", rownames(phi.fr))

# clean
phi.fr <- as.data.frame(phi.fr)
phi.fr$year <- as.numeric(gsub(".*?([0-9]+).*", "\\1", rownames(phi.fr)))
phi.fr$country <- gsub('[[:digit:]]+', '', rownames(phi.fr))
phi.fr$country <- gsub(".", "", phi.fr$country, fixed = TRUE)
phi.fr$ccode1 <- 220

# Save lambda/psi as one .csv file and save country ideal points as another .csv file
#write.csv(lambda,file="data/revise-latent-supp/lambda_fra.csv")
# write.csv(phi.fr, file="data/revise-latent-supp/country_irt_fra.csv")
