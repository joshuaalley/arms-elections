...
I then link defense contract awards and arms deals with a joint model.
Finally, I examine sectoral differences in weapons deals and contracting.
...
As a result, separate models of deals and contracting cannot capture the transfer of electoral arms deals with autocrats into defense contracts. 
...
In the following, I describe the joint model formally.
It starts with a model for the arms deals outcome $y_{it}^{deals}$. 
This is the above Poisson model of arms deals, so it includes an intercept $\alpha_{deals}$ and a matrix of the triple interaction of election timing, alliance and democracy as well as control variables $\textbf{X}$ with associated coefficient vector $\beta$. 

\begin{equation}
y_{it}^{deals} \sim Poisson(\mu_{it}^{deals}) 
\end{equation}

\begin{equation}
\mu_{it}^{deals} = \alpha_{deals} + \textbf{X} \beta
\end{equation}


The predicted arms deals for each country-year observation from the Poisson model $\mu_{it}^{deals}$ are also an independent variable in the defense contracts model. 
I include this by summing the country-year predicted outcomes in each year to create a variable $\mu_{t}^{deals}$. 
The contracts model uses a student-t likelihood with degrees of freedom $\nu$ and variance $sigma$. 
  

\begin{equation}
y_{st}^{cont} \sim Student(\mu_{st}, \sigma, \nu)
\end{equation}


The mean of annual contracts $\mu_{st}$ is a function state varying intercepts, lagged contracts, the interaction of the Global War on Terror with mean arms deals, and the control matrix \textbf{G}. 
Because the argument suggests that deals only drive contracting in periods with low demand, I account for the Global War on Terror, which increased defense spending through wars in Afghanistan and Iraq.  
To do this, I include an indicator of peak years in the global war on terror, which is one from 2001 to 2011, and zero otherwise.\footnote{I end the global war on terror variable at 2011 because the U.S. withdrawal from Iraq dramatically reduced its costs.} 
I then interact this dummy with the predicted mean of arms deals. 
$\rho_1$ is the key parameter for testing the contracts hypothesis, as this implies that greater deals increase contract awards during peacetime. 
$\rho_3$ captures the impact of deals during the general increase in contracting around the Iraq War, and should be close to zero.


In addition to the electoral competition and war variables, I include several controls. 
First, I adjust for population and GDP, in case larger and more prosperous states receive more contracts. 
I also control for electoral competition with indicators of whether a state is a swing or core state in the president's coalition \citep{KrinerReeves2015}.\footnote{Swing states are states where the losing party won at least 45\% of the two-party vote in three straight elections. 
...
I therefore include a state-specific lagged dependent variable, which allows temporal dependence in contracting to vary by state and is correlated with the state varying intercept. 
Formally, the state-level equation is: 

\begin{equation}
\mu_{st}^{cont} = \alpha_{state} + \theta_s y_{st-1} + \rho_1 \mu_{t}^{deals} + \rho_2 \mbox{GWOT} + \rho_3 \mu_{t}^{deals} * \mbox{GWOT} + \textbf{G} \lambda
\end{equation}



This model encapsulates the theoretical process. 
First, it assesses the impact of arms deals on defense contracting in war and peace. 
...
\begin{figure}[htpb]
	\centering
		\includegraphics[width=0.95\textwidth]{../figures/coef-joint-state.png}
	\caption{State-level coefficient estimates from a joint model of prime contracting awards to U.S. states between 2001 and 2020.}
	\label{fig:coef-joint-state}
\end{figure}



During the global war on terror, arms deals were associated with fewer contract awards, all else equal. 
While this was unexpected, it might reflect... 



A few controls are clearly correlated with contracting. 
While the log GDP estimate is largely positive, it includes some small negative values as well. 
Republican presidencies are not clearly associated with mean contract awards, but Republicans are associated with greater variance in contracting.
The positive correlation between Republican leaders and contract variance may occur because Republicans award more contracts to states with less economic weight and population that are core members of their  


This analysis supports the defense contracts hypothesis. 
In peacetime, swing states receive more defense contracts and contract awards to swing states are also more consistent.  
