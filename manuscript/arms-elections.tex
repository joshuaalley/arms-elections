...
Their defense industry may be smaller than less electorally competitive states like California, Texas, and New York, however. 
...
Delivery lags make arms transfer measures less useful than deals.
...
\begin{figure}[htpb]
	...
	\caption{Predicted changes in defense contracts during 2007/2008 presidential election, divided by electoral coalition status. Darker shading in the map indicates a larger increase in contracts, and white states are not part of the respective group. Points and colors mark the median estimate, while the error bars capture the 90\% credible interval. Estimates in millions of U.S. dollars.}
	\label{fig:map-08-full}
\end{figure}



The bottom row of \autoref{fig:map-08-full} presents the same estimates, but with uncertainty captured in 90\% credible intervals. 
...
The model predictions also pick up a large increase in contracts to Arizona, which was Republican nominee John McCain's home state.
...
These estimates are robust to a series of checks, including models with different control specifications, alternative estimators, and examining raw data.
...
Peacetime defense planning in NATO also makes Greece and Portugal a hard test for the political constraint mechanism.
...
Using the hurdle aids model fit because more country-year observations have zero deals for specific types of arms.
...
Increasing deals for these weapons are also positively correlated with swing state contracts.  
...
Deals for arms, vehicles, and ships, aircraft and electornic components have largely positive associations with corresponding swing state contracts.
Aircraft deals have the most important impact, because one extra aircraft deal is worth \$250 million more contracts, and there are more aircraft deals than any other sector. 
93\% of the posterior mass in the interaction of aircraft deals and swing state status is positive.
This reflects a diffuse supply chain for engines, airframes, and other components as well as the value of aircraft.


In addition to aircraft, arms, ships and electronic deals are correlated with greater swing state contracts. 
Arms and munitions go largely to swing states, and provide substantial benefits. 
Total annual ships deals range from one to 11. 
An additional ship deal is associated with \$300 million more contracts in a hypothetical swing state, so naval deals are rare but lucrative.
...
Electronics deals are marginally less lucrative than ships and aircraft, but are also relatively uncommon.
...
Budget cycles then offer a new explanation of when, why and to whom the United States sells arms. 
