...
To show how deals drive defense contracting, I use Department of Defense prime contract award data from the USAspending.gov database.\footnote{Link here: \url{https://www.usaspending.gov/download_center/custom_award_data}.} 
...
In the following, I describe the joint model formally.\footnote{Model specified and estimated with \textsf{brms} \citep{Buerkner2017}.}
...
\begin{equation}
y_{it} \sim Student(\mu_{st}, \sigma)
\end{equation}
...
$\beta_1$ is the key parameter for testing the contracts hypothesis, as this implies that greater deals increase contract awards during peacetime. 
$\beta_3$ captures the impact of deals during the general increase in contracting around the Iraq War, and should be close to zero.


\begin{equation}
\mu_{st}^{cont} = \alpha_{state} + \alpha_{year} + \theta_s y_{st-1} + \beta_1 \mbox{Deals} + \beta_2 \mbox{GWOT} + \beta_3 \mbox{Deals*GWOT} + \textbf{X} \beta
\end{equation}
